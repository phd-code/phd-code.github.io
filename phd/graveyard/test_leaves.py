...
plt.subplot(1,2,2)
#print "number of leaves",  tree.collect_leaf_keys_levels()
keys, levels, x, y, box_width = tree.collect_leaf_keys_levels()
current_axis = plt.gca()
for i in range(keys.size):
    tw = box_width[i]/2.0**order
    tx = x[i]/2.0**order
    ty = y[i]/2.0**order
    current_axis.add_patch(Rectangle((tx-.5*tw, ty-.5*tw), tw, tw, color=np.random.rand(3,1), alpha=0.5))
